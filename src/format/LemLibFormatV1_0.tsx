...
import { PointCalculationResult, fromDegreeToRadian, getPathPoints } from "../core/Calculation";
...
export function readWaypoint(buffer: SmartBuffer): LemLibWaypoint {
  ...
  if (flag & 0x01) {
    ...
    const deg = fromDegreeToRadian(rad);
...
export function writePathFile(buffer: SmartBuffer, paths: Path[], pathFileData: Record<string, any>) {
  ...
  buffer.insertUInt32LE(sizeOfBody, metadataStartIdx);
